"""
LLM-Guard detector adapter.

Calls containerized LLM-Guard service for ML-based prompt injection detection.
"""
...
LLMGUARD_SERVICE_URL = os.environ.get("LLMGUARD_SERVICE_URL", "http://localhost:8083")


class LLMGuardDetector(Detector):
    """Prompt injection detector using LLM-Guard library.

    Calls a containerized LLM-Guard service to isolate heavy ML dependencies.
    """
    ...
    def __init__(
        ...
        self._service_url = service_url or LLMGUARD_SERVICE_URL
    ...
    def detect(self, text: str, trust_level: TrustLevel = TrustLevel.USER) -> DetectionResult:
        ...
        if self._stub_mode:
            return DetectionResult(
                ...
                confidence=0.0,
                category=None,
                explanation="Stub mode: LLM-Guard disabled",
        ...
        if not self._service_available:
            return DetectionResult(
                ...
                confidence=0.0,
                category=None,
                explanation="Stub mode: LLM-Guard service unavailable",
                latency_ms=(time.perf_counter() - start_time) * 1000,
                detector_id=self.id,
            )
